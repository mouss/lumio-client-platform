...
- **Une route publique qui écrit en base doit échouer fermée.** Sans secret configuré, elle refuse tout. Une clé de test laissée en place est pire qu'une clé absente : elle laisse n'importe qui créer des données à distance. Vérifier au curl avec une signature forgée, et contrôler qu'aucune ligne n'a été créée.
...
- **2026-09-19, entrée des rendez-vous.** Moussa est sur le palier gratuit de Calendly, qui n'autorise pas les webhooks (l'API en lecture, elle, fonctionne sur tous les paliers). Décision : les fiches client se créent à la main avec le formulaire « Nouveau client », mode « RDV à venir », qui reproduit exactement ce que fait le webhook. Le webhook reste en place, testé, mais dormant, avec sa clé de signature vide pour échouer fermée. Une synchronisation par interrogation périodique de l'API Calendly a été proposée et écartée pour l'instant : elle est gratuite et faisable, elle reste l'option si la saisie manuelle devient pénible.
...
- **2026-09-17, Extension Second Cerveau.** Titre, accroche, 3 livrables et tarifs (500 € HT en upsell, 990 € HT en pack dédié) fournis mot pour mot par Moussa. Ils sont figés dans `lib/offres-contenu.ts` et repris dans une section « Extension Second Cerveau (option du Pack Quick Win) » ajoutée à `offres.md` dans le vault le 2026-09-17. Modalité de facturation : paiement en une fois, facture émise à la validation de l'offre par le client.
